...
This page outlines how to install the Custom Installer Builder (including its dependencies) on a Linux machine. We recommend you read up on [Seattle Infrastructure Architecture](../../UnderstandingSeattle/SeattleInfrastructureArchitecture.md) how the Seattle infrastructure architecture looks like so that you have a good understanding of the components you want to have (and thus need to set up).
...
First make sure you are logged in with your Custom Installer Builder account: `sudo -i -u cib`
Clone the Custom Installer Builder repository into `cib`'s home directory, and let the initialize script fetch dependencies:
...
Create the RepyV2 runtime, e.g. in `~/repy_runtime`, which will be needed by the Custom Installer Builder
...
Change to the `custominstallerbuilder` directory we checked out earlier and edit `local/settings.py` to match your local configuration:
...
...
ALLOWED_HOSTS = ["cib.loc"]
```

Add the parent directory of your deployed Custom Installer Builder and the Repy runtime directory to your `PYTHONPATH` and make sure `DJANGO_SETTINGS_MODULE` points to your local settings file.
...
*For testing purposes*, start the Django development webserver. This lets you check that your configs etc. have no syntax errors, and the Django app can be started. 
...
Before we build the base installers, create the directories where they will be placed into. The directories need to be accessible by your Django app, e.g.:
